...
const uni_modules_uvUiTools_libs_luchRequest_helpers_buildURL = require("../helpers/buildURL.js");
const uni_modules_uvUiTools_libs_luchRequest_core_buildFullPath = require("../core/buildFullPath.js");
const uni_modules_uvUiTools_libs_luchRequest_core_settle = require("../core/settle.js");
const uni_modules_uvUiTools_libs_luchRequest_utils = require("../utils.js");
const mergeKeys = (keys, config2) => {
  let config = {};
  keys.forEach((prop) => {
    if (!uni_modules_uvUiTools_libs_luchRequest_utils.isUndefined(config2[prop])) {
...
const adapter = (config) => {
  return new Promise((resolve, reject) => {
    let fullPath = uni_modules_uvUiTools_libs_luchRequest_helpers_buildURL.buildURL(uni_modules_uvUiTools_libs_luchRequest_core_buildFullPath.buildFullPath(config.baseURL, config.url), config.params, config.paramsSerializer);
    const _config = {
      url: fullPath,
      ...
      complete: (response) => {
        ...
        uni_modules_uvUiTools_libs_luchRequest_core_settle.settle(resolve, reject, response);
      }
    };
    let requestTask;
